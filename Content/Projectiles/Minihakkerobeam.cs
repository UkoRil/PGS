...
{
	public class Minihakkerobeam : ModProjectile
	{
		
		private const float PiBeamDivisor = MathHelper.Pi / Minihakkeroprojhold.NumBeams;
		...
		public override void AI() {
			...
			if (Projectile.type != ModContent.ProjectileType<Minihakkerobeam>() || !hostPrism.active || hostPrism.type != ModContent.ProjectileType<Minihakkeroprojhold>()) {
				Projectile.Kill();
				return;
			}
			...
			float chargeRatio = MathHelper.Clamp(hostPrism.ai[0] / Minihakkeroprojhold.MaxCharge, 0f, 1f);

			
			Projectile.damage = (int)(hostPrism.damage * GetDamageMultiplier(chargeRatio));

			
			Projectile.friendly = hostPrism.ai[0] > Minihakkeroprojhold.DamageStart;

			
			float beamIdOffset = BeamID - Minihakkeroprojhold.NumBeams / 2f + 0.5f;
			float beamSpread;
			float spinRate;
			...
			float deviationAngle = (hostPrism.ai[0] + beamIdOffset * spinRate) / (spinRate * Minihakkeroprojhold.NumBeams) * MathHelper.TwoPi;
		...
		private Color GetOuterBeamColor() {
			...
			float hue = (BeamID / Minihakkeroprojhold.NumBeams) % BeamHueVariance + BeamColorHue;
